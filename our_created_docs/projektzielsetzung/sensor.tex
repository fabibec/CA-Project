...
\begin{itemize}
    \item \textit{Submodul 0}: Startup \& Calibration Timer \\
    ...
    \item \textit{Submodul 1}: UART Receiver (FSM) \\
    ...
    Bei einem Fehler im Empfangsprozess wird ein Fehler-Flag gesetzt, und die IP beendet den Vorgang mittles des \textit{ap\_done} Signals.
    \item \textit{Submodul 2}: ASCII Decoder (FSM) \\
    Der ASCII Decoder empfängt stückweise die 5 ASCII-Zeichen im Format "Rxxx\textbackslash r", xxx bezeichnet hierbei eine ASCII-Zahl von 0-9.\\
    ...
    \item \textit{Submodul 3}: Distance Converter \\
...
\subsection{I/Os}
\begin{longtable}{|p{4cm}|p{0.75cm}|p{1.75cm}|p{8cm}|}
\hline
